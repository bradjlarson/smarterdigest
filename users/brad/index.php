...
mysql_select_db('jobs');
	if ($_SESSION['username'] == "brad")
	{
		if ($_SESSION['pwd'] == "jk109Hi")
		{
			$sql = "INSERT into test_user_access (user_name, access_type, access_time_stamp, session_id) values ('brad', 'Login Success', now(), '$sesh_id')";
			$exec = mysql_query($sql);
			$user_predictions = '/Dropbox/Coding/smarterdigest/engine/users/brad_predictions.php';
			include($user_predictions);
		}
		else
		{
			$sql = "INSERT into test_user_access (user_name, access_type, access_time_stamp, session_id) values ('brad', 'Login PWD Fail', now(), '$sesh_id')";
			$exec = mysql_query($sql);
...
<link rel="stylesheet" type="text/css" href="../../css/font-awesome.min.css">
	...
	function update_queue_count(prediction, vote) {
		var articles_in_queue = $("#articles_in_queue").val();
		articles_in_queue = parseInt(articles_in_queue);
		var likes_in_queue = $("#likes_in_queue").val();
		likes_in_queue = parseInt(likes_in_queue);
		var dislikes_in_queue = $("#dislikes_in_queue").val();
		dislikes_in_queue = parseInt(dislikes_in_queue);
