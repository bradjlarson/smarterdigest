...
mysql_select_db('jobs');
	if ($_SESSION['username'] == "zcs")
	{
		if ($_SESSION['pwd'] == "em879u4MrHgt")
		{
			$sql = "INSERT into test_user_access (user_name, access_type, access_time_stamp, session_id) values ('zcs', 'Login Success', now(), '$sesh_id')";
			$exec = mysql_query($sql);
			$user_predictions = '/Dropbox/Coding/smarterdigest/engine/users/zcs_predictions.php';
			include($user_predictions);
		}
		else
		{
			$sql = "INSERT into test_user_access (user_name, access_type, access_time_stamp, session_id) values ('zcs', 'Login PWD Fail', now(), '$sesh_id')";
			$exec = mysql_query($sql);
