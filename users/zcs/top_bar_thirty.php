...
where c.user_name = 'zcs'
and a.id > (c.starting_id - 30)) d, (select article_id, prediction, sum(ignore_flag) as num_ignore, sum(like_flag) as num_like from test_user_activity where user_name = 'zcs' group by 1 order by id desc limit 30) b
...
$sql = "SELECT prediction, like_flag, count(like_flag) as num_articles from test_user_activity where user_name = 'zcs' and like_flag is not null and prediction is not null group by 1,2 order by 1,2";
$result = mysql_query($sql);
$dislike_dislike = 0;
$dislike_like = 0;
$like_dislike = 0;
...
$sql = "SELECT star_flag, count(distinct article_id) as num_starred from test_user_activity where user_name = 'zcs' and star_flag = 1";
$result = mysql_query($sql);
$num_starred = 0;
while($row = mysql_fetch_array($result))
{
...
echo '<ul>' . PHP_EOL;
echo '   <li><a href="#"><span>Welcome, zcs</span></a></li>' . PHP_EOL;
