...
mysql_select_db('jobs');
	if ($_SESSION['username'] == "brad3")
	{
		if ($_SESSION['pwd'] == "")
		{
			$sql = "INSERT into test_user_access (user_name, access_type, access_time_stamp, session_id) values ('brad3', 'Login Success', now(), '$sesh_id')";
			$exec = mysql_query($sql);
			$user_predictions = '/Dropbox/Coding/smarterdigest/engine/users/brad3_predictions.php';
			include($user_predictions);
		}
		else
		{
			$sql = "INSERT into test_user_access (user_name, access_type, access_time_stamp, session_id) values ('brad3', 'Login PWD Fail', now(), '$sesh_id')";
			$exec = mysql_query($sql);
